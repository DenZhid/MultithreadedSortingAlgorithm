...
public class Main {
    ...
    private static final int[] testingSizes = {1000, 100_000, 1000_000, 10_000_000, 100_000_000};

    public static void main(String[] args) throws InterruptedException {
        for (int arrSize : testingSizes) {
            System.out.println("\nArrays size: " + arrSize);
            int[] testingArray = createArray(arrSize); // Заполнение массива заданной длины случайными значениями
            ...
            for (int threadPoolSize = 2; threadPoolSize <= 16; threadPoolSize *= 2) {
                ...
                System.arraycopy(
                        threads[threadPoolSize - 1].getSortableArr(),
                        0,
                        lastSubArr,
                        0,
                        arrSize - (threadPoolSize - 1) * subSize
                ...
                while (numberOfArraysForLastStage > 1) {
                    ...
                    mergers[numberOfArraysForCurrentStage - 1] =
                            new Merger(subArrays[(numberOfArraysForCurrentStage - 1) * 2], lastSubArr);
                    ...
                    System.arraycopy(
                            mergers[numberOfArraysForCurrentStage - 1].getRes(),
                            0,
                            lastSubArr,
                            0,
                            arrSize - currentSubSize * (numberOfArraysForCurrentStage - 1));
